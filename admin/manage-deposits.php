<?php
/* --------------------------------------------------
   AJAX HANDLER: MUST RUN BEFORE ANY HTML OUTPUT OR HEADERS
-------------------------------------------------- */
if (isset($_GET['action']) && $_GET['action'] === 'get_order_details') {
    ...
    try {
        // 1. Start output buffering to capture HTML markup generated by header.php
        ob_start();
        
        // 2. Include the header to leverage its session protection rules and $pdo initialization
        require_once __DIR__ . '/inc/header.php';
        
        // 3. Immediately purge the generated header HTML so it doesn't corrupt our strict JSON channel
        ob_clean(); 
        ...
        // Clean and explode values like "2,3" safely
...
/* --------------------------------------------------
   GLOBAL COMPONENT CONFIGURATION: STANDARD ROUTE VIEW
-------------------------------------------------- */
