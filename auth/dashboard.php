...
if (!isset($_SESSION['user_id'])) {
    $_SESSION['auth_error'] = "Please log in to see your dashboard.";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (!empty($full_name) && !empty($email_address)) {
        try {
            ...
            $error_message = "Something went wrong while saving your changes. Please try again.";
        }
    } else {
...
$admin_tickets = [
    ['file_path' => 'https://images.unsplash.com/photo-1540039155733-5bb30b53aa14?auto=format&fit=crop&w=600&q=80', 'description' => 'VIP Golden Circle Early Entry Pass. Valid for standard stadium layouts. Please save this ticket to your phone wallet for quick access at the gate.']
];
    ...
    try {
        ...
        foreach ($raw_txs as $tx) {
            $transaction_history[] = [
                ...
                'method'   => !empty($tx['image_path']) ? '../uploads/payment-methods/' . $tx['image_path'] : 'Payment Gateway',
                'amount'   => $tx['amount'],
                'currency' => 'USD',
    ...
    <?php if (isset($_SESSION['flash_error'])): ?>
        <div class="mb-6 bg-red-50 border-l-4 border-red-500 p-4 text-red-800 rounded-r-xl shadow-sm">
            <div class="flex items-center gap-2">
                ...
                <p class="font-bold">Payment Alert</p>
            </div>
            ...
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
                <div>
                    ...
                    <p class="text-blue-100 mt-2">
                        Welcome back, <span class="font-bold text-white"><?php echo htmlspecialchars($user_profile['name']); ?></span>! View your tickets, check your balance, and manage your profile here.
                    </p>
                ...
                <div class="flex gap-4 items-start shrink-0 relative group">
                    <button class="bg-white text-slate-900 font-bold px-6 py-3 rounded-xl shadow hover:shadow-lg transition flex items-center gap-2">
                        <i class="fas fa-headset text-blue-600"></i> Get Support
                    </button>
        ...
        <div class="bg-white rounded-2xl p-4 shadow mb-8 overflow-x-auto scrollbar-none">
            <div class="flex items-center space-x-2 min-w-max">
                ...
                <a href="#alerts-section" class="bg-slate-100 hover:bg-blue-600 text-slate-700 hover:text-white px-5 py-2.5 rounded-xl font-bold text-sm transition shadow-sm">
                    <i class="fas fa-bell mr-1.5"></i> Messages
                </a>
                <a href="#profile-section" class="bg-slate-100 hover:bg-blue-600 text-slate-700 hover:text-white px-5 py-2.5 rounded-xl font-bold text-sm transition shadow-sm">
                    <i class="fas fa-user-cog mr-1.5"></i> Account Settings
                </a>
                <a href="#orders-section" class="bg-slate-100 hover:bg-blue-600 text-slate-700 hover:text-white px-5 py-2.5 rounded-xl font-bold text-sm transition shadow-sm">
                    <i class="fas fa-shopping-bag mr-1.5"></i> Order History
                </a>
                <a href="#transactions-section" class="bg-slate-100 hover:bg-blue-600 text-slate-700 hover:text-white px-5 py-2.5 rounded-xl font-bold text-sm transition shadow-sm">
                    <i class="fas fa-receipt mr-1.5"></i> Billing History
                </a>
            </div>
        </div>
            ...
            <div class="lg:col-span-7 space-y-8">
                ...
                <div id="manifests-section" class="bg-white rounded-3xl shadow-xl p-8">
                    <h2 class="text-2xl font-black mb-2 flex items-center gap-2">
                        <i class="fas fa-ticket-alt text-indigo-600"></i> Digital Ticket Passes
                    </h2>
                    <p class="text-slate-500 mb-6">
                        Your ready-to-use event tickets for your upcoming shows.
                    </p>
                    <div class="space-y-4">
                            ...
                            <div class="group rounded-2xl border border-slate-200 bg-white hover:border-blue-600 hover:shadow-xl transition overflow-hidden">
                                ...
                                <div class="p-6">
                                    ...
                                    <a href="<?php echo htmlspecialchars($ticket['file_path']); ?>" target="_blank" class="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl px-6 py-3 font-bold transition shadow shadow-blue-600/20 text-xs uppercase tracking-wider">
                                        <i class="fas fa-download"></i> View / Download Ticket
                                    </a>
                                </div>
                            </div>
                        <?php endforeach; ?>
                ...
                <div id="orders-section" class="bg-white rounded-3xl shadow-xl p-8">
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            ...
                            <p class="text-slate-500 text-sm mt-1">Check out your active tickets and passes.</p>
                        </div>
                            ...
                            ?>
                                <div class="rounded-2xl border border-slate-200 p-5 bg-white hover:border-slate-300 transition flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                                    ...
                                    <div class="text-left sm:text-right shrink-0 border-t sm:border-t-0 border-slate-100 pt-3 sm:pt-0">
                                        ...
                                        <span class="text-[11px] text-slate-400 font-medium">Purchase Date</span>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <p class="text-sm text-slate-400 italic text-center py-6">You don't have any active tickets yet.</p>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div id="transactions-section" class="bg-white rounded-3xl shadow-xl p-8">
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            <h2 class="text-2xl font-black">Payment History</h2>
                            <p class="text-slate-500 text-sm mt-1">A quick log of your recent balance top-ups.</p>
                        </div>
                    ...
                    <div class="overflow-x-auto rounded-2xl border border-slate-200">
                                        ...
                                        <tr class="hover:bg-slate-50/80 transition">
                                            ...
                                            <td class="p-4 text-slate-500 font-bold">
                                                <?php if (strpos($txn['method'], '../uploads/') === 0): ?>
                                                    <img src="<?php echo htmlspecialchars($txn['method']); ?>" alt="Payment Method" class="h-6 w-auto object-contain rounded max-w-[60px]">
                                                <?php else: ?>
                        ...
                        <?php else: ?>
                            <p class="text-sm text-slate-400 italic text-center py-6">No deposits recorded yet.</p>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <div class="lg:col-span-5 space-y-8 sticky top-6">
                    ...
                    <div class="space-y-5">
                        <div class="flex justify-between border-b border-white/10 pb-4">
                            <div>
                                ...
                                <div class="text-sm text-white/60">Your display name</div>
                            </div>
                        ...
                        <div class="flex justify-between border-b border-white/10 pb-4">
                            <div>
                                ...
                                <div class="text-sm text-white/60">Your primary email</div>
                            </div>
                ...
                <div id="alerts-section" class="bg-white rounded-3xl shadow-xl p-8">
                    <h3 class="text-xl font-black text-slate-900 mb-2 flex items-center gap-2">
                        <i class="fas fa-bell text-amber-500 animate-pulse"></i> Notice Board
                    </h3>
                    <p class="text-slate-500 text-xs mb-6">Important updates and direct notices from the admin team.</p>
                    ...
                    <div class="space-y-4 max-h-[300px] overflow-y-auto pr-1">
                        <?php if (!empty($admin_messages)): ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <p class="text-xs text-slate-400 italic text-center py-6 bg-slate-50 border border-dashed border-slate-200 rounded-xl">No new alerts or messages.</p>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div id="profile-section" class="bg-white rounded-3xl shadow-xl p-8">
                    ...
                    <p class="text-slate-400 text-xs mb-6">Update your personal contact details here.</p>
